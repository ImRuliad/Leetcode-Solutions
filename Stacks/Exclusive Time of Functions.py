...
class Solution:
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        ...
        for log in logs:
            log = log.split(":")
            id, op, timestamp = int(log[0]), log[1], int(log[2])

            if op == "start":
                if stack:
                    call_at_stack_head: int = stack[-1][0]
                    time_at_stack_head: int = stack[-1][1]
                    exclusive_time: int = timestamp - time_at_stack_head
                    result[call_at_stack_head] += exclusive_time
                start_stats: List[int] = [id, timestamp]
                stack.append(start_stats)
            if op == "end":
                end_stats: List[int] = stack.pop()
                end_of_runtime: int = end_stats[1] + 1
                time_diff: int = timestamp - end_stats[1] + 1
                result[id] += time_diff
                if stack:
